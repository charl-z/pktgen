...
int CNetRecv::init_peer_ipinfo(uint32_t ipaddr, uint16_t port)
{
    ...
    char *str = ip_to_str(htonl(ipaddr));
    if (NULL == str) 
    {
    ...
    strncpy(m_ipstr, str, HOST_IP_LEN);
    return 0;
}
...
int CNetRecv::init_peer_ipinfo(char *ipstr, uint16_t port)
{
    ...
    if (0 != str_to_ip(ipstr, &m_ipaddr))
    {
        _LOG_ERROR("Invalid server %s", ipstr);
        return -1;
    }
    m_ipaddr = ntohl(m_ipaddr);
    return 0;
}
...
int CNetRecv::init_local_ipinfo()
{
    ...
    char *str = ip_to_str(htonl(m_local_ipaddr));
    if (NULL == str) 
    {
        _LOG_ERROR("ip to str failed.");
        return -1;
    }
    strncpy(m_local_ipstr, str, HOST_IP_LEN);
