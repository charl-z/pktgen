					...
					BOOL isOnce)
{
	...
	m_time_value = time_value_ms;
	m_is_once = isOnce;

	m_expire_time = util_get_cur_time_ms() + time_value_ms;
}
...
void CTimeJobMgr::begin(unsigned int thrd_index)
{
	...
	this->m_latest_expire_time = util_get_cur_time_ms();
}
...
void CTimeJobMgr::run(unsigned int thrd_index)
{
	...
	uint64_t nowtime = util_get_cur_time_ms();
	unsigned int eclapse_time = nowtime - this->m_latest_expire_time;
	if (eclapse_time >= 1)
	{
        /*10ms检查一次*/
		this->time_job_walk(nowtime);
        this->m_latest_expire_time = nowtime;
	}
}
...
BOOL CTimeJobMgr::add_time_job(expire_hdl_func expire_func,
					...
					BOOL isOnce)
{
	CTimeJob *timeNode = new CTimeJob(expire_func, param1, param2, param3, param4, time_value_ms, isOnce);
	assert(timeNode);
