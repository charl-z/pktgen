...
#define EVENTMAX 1024
	...
	while(false == pollObj->m_isShutDown)
	{
		...
		fd_num = epoll_wait(pollObj->m_epfd[evt_fd_index], events, EVENTMAX, EPOLL_TIMEOUT);
						...
						int bufferSize, BOOL isTcp)
{
	...
	if (NULL == job_node)
	{
		...
		if(epoll_ctl(m_epfd[thrd_index], EPOLL_CTL_ADD, fd, &ev) != 0)
		{
			...
			delete job_node;
			_LOG_ERROR("epoll_ctl add read failed, thrd idx %d, fd %d, %s.", fd,
					 thrd_index, str_error_s(err_buf, sizeof(err_buf), errno));
			return false;
		}
#endif
		job_node->unlock();
						...
						BOOL isTcp)
{
	...
	if (NULL == job_node)
	{
		...
		if(epoll_ctl(m_epfd[thrd_index], EPOLL_CTL_ADD, fd, &ev) != 0)
		{
			...
			delete job_node;
			_LOG_ERROR("epoll_ctl add write failed, thrd idx %d, fd %d, %s.", fd,
					thrd_index, str_error_s(err_buf, sizeof(err_buf), errno));
			return false;
		}
#endif
	...
	if (false == job_node->io_event_write())
	{
		...
		if (job_node->io_event_read())
		{
			...
			if(epoll_ctl(m_epfd[job_node->get_thrd_index()], EPOLL_CTL_MOD, fd, &ev) != 0)
			{
				_LOG_ERROR("epoll_ctl mod to write failed, thrd idx %d, fd %d, %s.",
									job_node->get_thrd_index(), fd,	str_error_s(err_buf, sizeof(err_buf), errno));
				job_node->unlock();
				g_IoJobMgr->unlock();
				return false;
			}
		...
		else
		{
			...
			{
				_LOG_ERROR("epoll_ctl add write failed, thrd idx %d, fd %d, %s.", fd,
						job_node->get_thrd_index(), str_error_s(err_buf, sizeof(err_buf), errno));
				job_node->unlock();
				g_IoJobMgr->unlock();
				return false;
			}
